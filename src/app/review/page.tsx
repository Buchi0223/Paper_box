...
export default function ReviewPage() {
  ...
  const fetchPapers = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/papers/review?sort=${sortOrder}&limit=50`,
      );
      if (res.ok) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (isActioning || papers.length === 0 || currentIndex >= papers.length)
        return;
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            未レビュー: {remaining > 0 ? remaining : 0}件
          </p>
        </div>
        ...
        <>
          ...
          <div className="mx-auto max-w-2xl">
            <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
              {/* スコア表示 */}
              {currentPaper.relevance_score !== null &&
                currentPaper.relevance_score !== undefined && (
                  <div className="mb-4 flex items-center gap-2">
                    <span
                      className={`rounded-full px-3 py-1 text-sm font-bold ${
                        currentPaper.relevance_score >= 70
                          ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"
                          : currentPaper.relevance_score >= 40
                            ? "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400"
                            : "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"
                      }`}
                    >
                      スコア: {currentPaper.relevance_score}
                    </span>
                  </div>
                )}
              ...
              <h2 className="mb-2 text-xl font-bold leading-snug text-gray-900 dark:text-white">
                {currentPaper.title_ja || currentPaper.title_original}
              </h2>
              {currentPaper.title_ja && (
                <p className="mb-4 text-sm text-gray-500 dark:text-gray-400">
                  {currentPaper.title_original}
                </p>
              )}
                ...
                {currentPaper.authors.length > 0 && (
                  <span>
                    {currentPaper.authors.length <= 3
                      ...
                      : `${currentPaper.authors.slice(0, 3).join(", ")} et al.`}
                  </span>
                )}
                {currentPaper.published_date && (
                  <>
